...
trait CirceEncoders {
  ...
  implicit object DataTypeEncoder extends Encoder[DataType] {
    override def apply(a: DataType): Json = a.toString.asJson
  }
  ...
  implicit val dataSourceEncoder: Encoder[DataSource] = Encoder.instance {
    case jdbc: JdbcSource ⇒ jdbc.asJson
  }
...
trait CirceDecoders {
  ...
  implicit val dataTypeDecoder: Decoder[DataType] = Decoder.decodeString.map{ str ⇒
  ...
  implicit val eqlDecoder: Decoder[eql] = new Decoder[eql] {
    override def apply(c: HCursor): Result[eql] = {
      for {
        dimension ← c.downField("dimension").as[Dimension]
        value ← c.downField("value").as[String]
        includeOpt ← c.downField("include").as[Option[Boolean]]
      } yield {
        eql(dimension, value, includeOpt.getOrElse(true))
      }
    }
  }
  ...
  implicit val dataSourceDecoder: Decoder[DataSource] = {
    Decoder[JdbcSource].map[DataSource](identity)
  }
}
